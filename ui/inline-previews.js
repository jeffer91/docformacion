(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .inline-preview-slot{margin:12px 0 0 41px;border:1px solid #dfe5ef;border-radius:12px;overflow:hidden;background:#eef2f6;min-height:460px;position:relative}
      .document-element-card .inline-preview-slot{margin:4px 0 0;min-height:390px}
      .inline-preview-frame{display:block;width:100%;height:560px;border:0;background:#fff}
      .document-element-card .inline-preview-frame{height:500px}
      ...
      @media(max-width:820px){
        .inline-preview-slot{margin-left:0;min-height:390px}
        .inline-preview-frame{height:470px}
        .document-element-card .inline-preview-frame{height:430px}
        .inline-preview-loading,.inline-preview-error{min-height:390px}
      }
    `;
  ...
  async function buildPreview(slot) {
    ...
    loading.add(key);
    slot.dataset.previewLoading = 'true';
    try {
      let result;
      if (kind === 'element') {
      ...
      slot.innerHTML = `
        <iframe class="inline-preview-frame" title="Vista previa directa del documento" src="${url}#toolbar=0&navpanes=0&scrollbar=1&view=FitH"></iframe>
        <div class="inline-preview-caption"><strong>Vista previa directa del PDF</strong><span>${status} · ${pages} página${pages === 1 ? '' : 's'}</span></div>`;
      slot.dataset.previewReady = 'true';
    } catch (error) {
      if (slot.isConnected) {
        slot.innerHTML = `<div class="inline-preview-error">No se pudo generar esta vista previa. ${clean(error?.message || error)}</div>`;
      }
    } finally {
      delete slot.dataset.previewLoading;
      loading.delete(key);
    }
  }

  function observeSlot(slot) {
    if (!slot || slot.dataset.previewReady === 'true' || slot.dataset.previewLoading === 'true') return;
  ...
  function refreshCopy() {
    ...
    if (workspaceCopy) workspaceCopy.textContent = 'Cada sección muestra directamente la misma salida que se genera en PDF.';
